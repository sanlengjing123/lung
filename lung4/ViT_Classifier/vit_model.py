...
def vit_large_patch32_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    # 添加新的弱分类器
    model.weak_classifier = nn.Sequential(
        nn.Linear(1024, 768),  # 输入维度应为嵌入维度
        nn.Tanh(),
        nn.Linear(768, num_classes)  # 输出维度应为类别数量
    )
    
    # 修改 forward 方法以包含弱分类器
    original_forward = model.forward

    def modified_forward(x):
        x = model.forward_features(x)
        main_output = model.head(x)
        weak_output = model.weak_classifier(x)
        return main_output, weak_output
